...
export const apiGetHomeScheduleItems = (queryDate, queryDir, queryStop, bstopTo) => {
  ...
  return axios.get(
    API_ROOT_PATH +
      `/api/scheduleItem/home-from?qdate=${queryDate}&qdir=${queryDir}&qbstopfrom=${queryStop}&qbstopto=${bstopTo}`
  );
  // else
...
export const apieGetSchedulesByRule = (ruleId) => {
  return axios.get(`${API_ROOT_PATH}/api/scheduleItem/get-schedules-by-rule?ruleId=${ruleId}`, {
    headers: authHeader(),
  });
};
